    ...
    for icall_idx in icall_indices:
        ...
        while j >= 0:
            ...
            if (re.match(r'^loc_[0-9A-Fa-f]+:', stripped) or
                ...
                re.match(r'^PUSH32\(esp, 0\); (sub|fn)_[0-9A-Fa-f]{8}', stripped)):
                break
...
class FunctionTranslator:
    ...
    def translate_function(self, func_addr, func_info):
        ...
        symbol_name = func_info.get("symbol_name", "")
        size = end - start
...
class BatchTranslator:
    ...
    def translate_batch(self, func_list, output_file=None, max_funcs=None,
        ...
        c_chunks = []
        c_chunks.append("/**")
        c_chunks.append(" * Xbox game - Mechanically Translated Game Code")
    ...
    def translate_batch_split(self, func_list, output_dir, chunk_size=1000,
        ...
        # Some optimized Xbox binaries branch into addresses that were not
        # accepted as function starts even after seed feedback. Declare and
        # define loud fallback stubs for those residual targets so the first
        # generated C build can proceed and runtime logs identify the bad edge.
        import re
        translated_names = {name for _, name, _ in translations}
        referenced_names = set()
        func_ref_re = re.compile(r"\b((?:sub|fn)_([0-9A-Fa-f]{8})[A-Za-z0-9_]*)\s*\(")
        for _, _, code in translations:
            referenced_names.update(match.group(1) for match in func_ref_re.finditer(code))
        missing_names = sorted(referenced_names - translated_names,
                               key=lambda n: int(re.match(r"^(?:sub|fn)_([0-9A-Fa-f]{8})", n).group(1), 16))
        for name in missing_names:
            addr = int(re.match(r"^(?:sub|fn)_([0-9A-Fa-f]{8})", name).group(1), 16)
            stub = (
                f"/* Fallback for unresolved generated target 0x{addr:08X}. */\n"
                f"void {name}(void)\n"
                "{\n"
                f"    recomp_missing_target(0x{addr:08X}u);\n"
                "}\n"
            )
            translations.append((addr, name, stub))
        stats["fallback_stubs"] = len(missing_names)

        # Generate header with all forward declarations
        header_path = os.path.join(output_dir, header_name)
        header_lines = [
            "/**",
            " * Xbox game - Recompiled Function Declarations",
        ...
        for ci, chunk in enumerate(chunks):
            ...
            c_lines = [
                "/**",
                f" * Xbox game - Recompiled code chunk {ci}",
    ...
    def _write_dispatch_table(self, translations, output_path, header_name):
        ...
        lines = [
            "/**",
            " * Xbox game - Recompiled Function Dispatch Table",
